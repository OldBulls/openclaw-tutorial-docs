---
title: 先读我
description: 3 分钟了解 OpenClaw 实战手册的内容结构、适用人群和阅读路径，决定要不要继续读。
---
...
```bash
# 1. 装 CLI（需要 Node 22.10+；教程当前公开稳定包基线为 2026.5.2）
...
每条命令、每个路径、每段配置，作者都在自己的生产环境跑过。所有文件的重写状态追踪见 [仓库维护清单](https://github.com/OldBulls/openclaw-tutorial/blob/main/docs/_REWRITE_STATUS.md)：

- 🟢 **verified** 已基于真实 CLI + 实测文件重写

当前公开章节已全部收口到 `🟢`，每次发版前都会按 `meta/RELEASE_CHECKLIST.md` 跑一遍 audit。
...
03-多Agent/00-协作拓扑 ← 多 agent 设计原则
  ↓
07-实战/00-最近新增能力总览 ← 新增能力路线图 + 飞书卡片实战
  ↓
分叉
  ├── 继续深入 docs/ 自己搭
...
如果你主要想做“飞书里的 AI 工作流面板”，跑通入门和飞书插件后，可以直接进入新增的 [07-实战/00-最近新增能力总览](./07-实战/00-最近新增能力总览.md)，再按顺序做交互卡片、周审卡片、回调 handler 和排障验证。
...
本教程基于 **OpenClaw 2026.5.2 公开稳定包基线** 持续更新，最后更新 **2026-05-09**。作者端已通过蓝绿流程验证 OpenClaw `2026.5.7` 与 `openclaw-lark 2026.5.7`，但公开稳定安装包未发新版前仍以 `latest.json` 和 `RUNTIME_BASELINE.json` 为准。

OpenClaw 迭代快，升级后建议：

1. 看 [OpenClaw 更新日志](https://github.com/openclaw/openclaw/blob/main/CHANGELOG.md)
2. 先看 [06-升级/03-CLI与Gateway版本统一](./06-升级/03-CLI与Gateway版本统一.md)，确认 CLI 和 gateway 服务不是两套安装
3. 跑 `openclaw doctor` 确认兼容
4. 若 doctor 报错，查 `06-升级/00-npm升级兼容` 章节（含运行时补丁重打和锚点失配处理）

> 2026-05-09 更新说明：当前公开稳定包是 `v1.0.13 / OpenClaw 2026.5.2`；作者端已验证 `2026.5.7`，并把 `memory-lancedb-pro` 的 5.7 编译入口和 `openclaw-lark 2026.5.7` patch 校验预备进模板脚本。想看新增能力路线图，先读 [07-实战/00-最近新增能力总览](./07-实战/00-最近新增能力总览.md)。
